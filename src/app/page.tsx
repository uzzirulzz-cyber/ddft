...
const MODE_META: Record<Mode, { label: string; subtitle: string }> = {
  trading: { label: "Trading", subtitle: "Place orders, track positions, manage your portfolio" },
  admin: { label: "Admin", subtitle: "Exchange operations & risk controls" },
};
        ...
        {mobileNavOpen && (
          <>
            ...
            <motion.aside
              ...
              className="lg:hidden fixed left-0 top-0 z-50 w-64 h-screen bg-sidebar border-r border-sidebar-border flex flex-col"
            >
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="header-backdrop sticky top-0 z-30 flex h-16 items-center gap-3 px-4 lg:px-6">
          <Button
            variant="ghost"
            size="icon"
            className="lg:hidden h-9 w-9 hover:bg-sidebar-accent"
          ...
          <div className="hidden md:flex flex-col min-w-0">
            <h1 className="text-lg font-bold leading-tight truncate tracking-tight">{meta.title}</h1>
            <p className="text-xs text-muted-foreground truncate">{meta.subtitle}</p>
          </div>

          {/* Desktop mode switcher (topbar) */}
          <div className="hidden lg:flex items-center gap-1 mx-auto rounded-lg bg-sidebar-accent/60 p-1 border border-sidebar-border">
            <ModeButton
              mode="trading"
              current={mode}
          ...
          <div className="ml-auto flex items-center gap-2 lg:gap-3">
            <div className="relative hidden sm:block">
              ...
              <Input
                ...
                className="pl-9 h-9 w-44 md:w-64 bg-sidebar-accent/60 border-sidebar-border placeholder:text-muted-foreground"
              />
            </div>

            <Button variant="ghost" size="icon" className="relative h-9 w-9 hover:bg-sidebar-accent">
              <Bell className="h-5 w-5" />
              <span className="absolute top-1.5 right-1.5 h-2 w-2 rounded-full bg-amber-500 pulse-dot" />
            </Button>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="flex items-center gap-2 rounded-full pl-1 pr-2 py-1 hover:bg-sidebar-accent transition-colors">
        ...
        {/* Ticker tape — only in trading mode */}
        {mode === "trading" && <TickerTape />}

        {/* Mobile section title + mode switcher */}
        <div className="md:hidden px-4 pt-4 space-y-3">
          <div className="flex items-center gap-1 rounded-lg bg-sidebar-accent/60 p-1 border border-sidebar-border">
            <ModeButton
              mode="trading"
              current={mode}
              onClick={() => setMode("trading")}
        ...
        {/* Footer */}
        <footer className="mt-auto border-t border-sidebar-border bg-[#020617]/60 px-4 lg:px-6 py-4">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-muted-foreground">
            <p>
              © 2026 Brock Exchange ·{" "}
              <span className="text-amber-500 font-medium">Trade Smarter.</span>{" "}
              <span className="text-blue-400 font-medium">Grow Faster.</span>
            </p>
...
function SidebarContent({
  ...
  return (
    <>
      <div className="flex h-16 items-center gap-2 border-b border-sidebar-border px-5 header-backdrop">
        <Brand />
      </div>
      <ModeSwitcher mode={mode} setMode={setMode} />
      {mode === "admin" ? (
      ...
      <div className="mt-auto p-4 border-t border-sidebar-border">
        {mode === "trading" ? (
          <div className="rounded-xl border border-amber-500/20 p-4 bg-gradient-to-br from-amber-500/15 to-amber-500/5">
            <p className="text-xs font-semibold text-amber-400">Trading Mode</p>
            <p className="mt-1 text-xs text-muted-foreground leading-relaxed">
              Live order book, candlestick charts, and one-click buy/sell. Switch to Admin to manage the exchange.
            </p>
          </div>
        ) : (
          <div className="rounded-xl border border-blue-500/20 p-4 bg-gradient-to-br from-blue-500/15 to-blue-500/5">
            <p className="text-xs font-semibold text-blue-400">System Status</p>
            <p className="mt-1 text-xs text-muted-foreground leading-relaxed">
...
function ModeSwitcher({ mode, setMode }: { mode: Mode; setMode: (m: Mode) => void }) {
  return (
    <div className="px-3 pt-3">
      <div className="grid grid-cols-2 gap-1 p-1 bg-sidebar-accent/60 rounded-lg border border-sidebar-border">
        <ModeButton
          mode="trading"
          current={mode}
          onClick={() => setMode("trading")}
...
function NavList({
  ...
  return (
    <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
      <p className="px-3 pb-2 text-[10px] uppercase tracking-wider text-muted-foreground/70 font-semibold">
        Admin Menu
      </p>
      {NAV_ITEMS.map((item) => {
    ...
    <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
      ...
      {items.map((item) => {
        ...
        return (
          ...
          >
            <Icon className={`h-4 w-4 ${item.active ? "text-amber-500" : ""}`} />
            <span className="flex-1 text-left">{item.label}</span>
            {item.active && (
              <motion.div
