    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 z-[100] flex items-center justify-center bg-[#02060f]/90 backdrop-blur-md p-4"
        >
          <motion.div
            initial={{ scale: 0.92, y: 20 }}
            animate={{ scale: 1, y: 0 }}
            exit={{ scale: 0.92, y: 20 }}
            className="bx-glass rounded-2xl p-8 max-w-md w-full bx-glow-strong relative"
          >
            <div className="text-center mb-5">
              ...
              <p className="mt-1.5 text-sm text-muted-foreground">
                For your security, please change the default password before continuing.
              </p>
            </div>
            ...
            <form onSubmit={submit} className="space-y-3">
              ...
              <Button type="submit" disabled={loading} className="w-full bx-blue-gradient bx-glow text-white border-0 h-11">
            ...
            <button onClick={handleLogout} className="block mx-auto mt-4 text-xs text-muted-foreground hover:text-white">
              Logout instead
            </button>
          </motion.div>
        </motion.div>
